...
with DAG(
    ...
    schedule_interval='0 0 1 * *',  # At 00:00 on day-of-month 1 ## in cron, can google 
    ...
    dep_check_source_label_data = DummyOperator(task_id="dep_check_source_label_data") ## Dummy task. In actual, have to choose other operator. 

    bronze_label_store = BashOperator( ## This is the task. Can use BashOPerator, PythonOperator, any operators suitable 
        task_id='run_bronze_label_store',
        bash_command=(
            'cd /opt/airflow/scripts && '
            'python3 bronze_label_store.py '
            '--snapshotdate "{{ ds }}"' ## {{ ds }} this is where you input the date (think back the for loop in assignment 1 or lab where we generate the list)
        ),
    )
    ...
    ## >> represents the workflow. Chain your tasks  
