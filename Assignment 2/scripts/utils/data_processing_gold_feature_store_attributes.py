...
def process_gold_features(table_name, input_dir, output_dir, spark):
    ...
    # Expand Occupation_ohe
    df = df.withColumn("Occupation_ohe_array", vector_to_array("Occupation_ohe"))
    n_occupation = df.selectExpr("size(Occupation_ohe_array)").first()[0]
    for i in range(n_occupation):
        df = df.withColumn(f"Occupation_ohe_{i}", col("Occupation_ohe_array")[i])
    ...
    df = df.select('Customer_ID', 'Age', *[f"Occupation_ohe_{i}" for i in range(n_occupation)])
