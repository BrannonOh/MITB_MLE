...
def process_gold_features(table_name, input_dir, output_dir, spark):
    ...
    # Expand Credit_Mix_ohe
    df = df.withColumn("Credit_Mix_ohe_array", vector_to_array("Credit_Mix_ohe"))
    n_credit_mix = df.selectExpr("size(Credit_Mix_ohe_array)").first()[0]
    for i in range(n_credit_mix):
        df = df.withColumn(f"Credit_Mix_ohe_{i}", col("Credit_Mix_ohe_array")[i])

    # Expand Payment_of_Min_Amount_ohe
    df = df.withColumn("Payment_of_Min_Amount_ohe_array", vector_to_array("Payment_of_Min_Amount_ohe"))
    n_payment = df.selectExpr("size(Payment_of_Min_Amount_ohe_array)").first()[0]
    for i in range(n_payment):
        df = df.withColumn(f"Payment_of_Min_Amount_ohe_{i}", col("Payment_of_Min_Amount_ohe_array")[i])

    # Expand Payment_Behaviour_ohe
    df = df.withColumn("Payment_Behaviour_ohe_array", vector_to_array("Payment_Behaviour_ohe"))
    n_behavior = df.selectExpr("size(Payment_Behaviour_ohe_array)").first()[0]
    for i in range(n_behavior):
        df = df.withColumn(f"Payment_Behaviour_ohe_{i}", col("Payment_Behaviour_ohe_array")[i])
    ...
    df = df.select('Customer_ID', 'Annual_Income', 'Monthly_Inhand_Salary', 'Num_Bank_Accounts', 'Num_Credit_Card', 
                   ...
                   *[f"Credit_Mix_ohe_{i}" for i in range(n_credit_mix)], 
                   *[f"Payment_of_Min_Amount_ohe_{i}" for i in range(n_payment)],
                   *[f"Payment_Behaviour_ohe_{i}" for i in range(n_behavior)])
